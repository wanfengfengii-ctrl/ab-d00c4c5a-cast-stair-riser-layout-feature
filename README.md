...
- **中间标高控制点**：现场复测常得到平台下口或转折级的已知累计标高。确定踏步数后（自动推荐或人工选用），
  请求可携带若干控制点 `(k, e)`：第 `k` 级（`1 ≤ k ≤ n−1`）处的累计标高为 `e`（`0 < e < H`，正整数毫米），
  且各点按提交顺序级号、标高均严格递增。计算端把起点 `(0, 0)`、各控制点、层高终点 `(n, H)` 作为锚点**按级号分段**：
  对级号 `a→b`、标高 `ea→eb` 的一段（段级数 `m = b−a`，段高差 `Δ = eb−ea`），段内第 `j` 级相对段起点的
  累计增量取 `ceil_0.5(j·Δ / m)`（**按半毫米向上取整**，即半毫米网格上的 `⌈2·j·Δ/m⌉/2`）。
  于是段末 `j = m` 时增量恰为 `Δ`，**控制点精确命中**；段内任一累计值相对该段理想直线的误差位于 `[0, 0.5mm)`，
  同组控制点结果按级号**唯一确定**。受控后每级踏步高度仍必须落入踏步高度闭区间 `[hmin, hmax]`，
  否则返回可定位到具体控制点的 `422`，并说明越界级与该级计算高度。
...
`control_points` 可选，为中间标高控制点数组（空数组等价于不带）。每项的 `step`、`cumulative_mm` 均为严格模式下的正整数（拒绝浮点、字符串、布尔）。控制点在踏步数确定后校验：`step` 须位于该方案首末级之间（`1..n−1`），`cumulative_mm` 须位于 `0` 与层高之间，且各项级号、标高按数组顺序严格递增；受控后任一单级高度越出 `[hmin, hmax]` 闭区间时同样返回 `422`，错误定位于 `body.control_points.<下标>.step|cumulative_mm`，消息含越界级号与该级计算高度。旧请求不带 `control_points` 时，响应与余数前置序列**完全不变**。
...
- `solution`（仅 `ok` 时）：`steps`（**当前选用**的踏步数，自动时等于推荐值，人工时等于请求的 `selected_steps`）、`treads`、`exact_riser_mm`、
  ...
  受控时额外含 `controlled: true` 与 `control_points`（各项 `{step, requested_mm, hit_mm, error_mm}`，命中误差恒为 0）；
  受控序列落在半毫米网格上，逐级高度/累计标高可能为 `x.5`，余数前置说明不再适用。
- `recommended_steps`：自动推荐踏步数（人工选用或受控时也保留）；`no_solution` 时为 `null`。
- `selection_source`：`"auto"`（未携带 `selected_steps`）或 `"manual"`（人工选用成功，与是否受控无关）；`no_solution` 时为 `null`。
- `candidates`：2–40 级全部候选的精确高度/深度、是否可行、`recommended`（推荐标记，人工改选/受控后仍在原推荐行）、`selected`（当前选用）及淘汰原因。
...
- 结论区提供“中间标高控制点”录入：自动推荐或人工选用踏步数后，可录入若干“级号、累计标高”（级号取 `1..n−1`、标高取 `0..H`，严格递增）并点击“应用控制点”。成功后逐级表按控制点分段重新生成（半毫米向上取整），结论处显示“控制点受控”标识、逐级表在受控级标注“控制点”并列出各控制点命中值；自动/人工来源与候选推荐、选中标记全部保留。应用失败（含单级高度越出闭区间的 `422`）时错误定位到具体行字段并保留当前有效方案。
- 任一尺寸变化或改选踏步数都会**清空控制点**（录入内容与受控状态），按原余数前置链路重新计算；人工选用的清除规则保持不变，改选请求失败仍保留当前方案并原位提示。
- 无候选时页面**只**展示明确的无法放样结论，不展示方案与候选表，人工选用与控制点录入均无入口。
